...
if(isset($_FILES)) { //Check to see if a file is uploaded
    try {
        ...
        /*foreach($newArr as $nArray){
            $hours += $nArray['hours'];
        }*/
        //var_dump($newArr);
        $count = count($newArr);
        $output = $exceptions = $tracking = array();
        foreach($newArr as $key => $nArray){
        ...
        if(strpos($_SESSION['originalFileName'], "Deming") !== false){
            ...
            $originalFileName = $temp[1];
        }elseif(strpos($_SESSION['originalFileName'], "Chaparral") !== false){
            $temp = explode(" ", $_SESSION['originalFileName']);
            $originalFileName = $temp[1];
        }elseif(strpos($_SESSION['originalFileName'], "Las Cruces") !== false){
            $temp = explode(" ", $_SESSION['originalFileName']);
            $originalFileName = $temp[1]. "-". $temp[2];
        }else{
            $temp = explode(" ", $_SESSION['originalFileName']);
            $originalFileName = $temp[1] . "-" . $temp[2];
        }
        ...
        $fileName = "EvoFiles/".$originalFileName. "-AtHome_Evo_File-" . $month . "-" . $day . "-" . $year . ".csv";
        ...
        if(count($exceptions) > 0) {
            $exFileName = "ExceptionFiles/".$originalFileName. "-AtHome_Exception_File-" . $month . "-" . $day . "-" . $year . ".csv";
        ...
        $trackingFileName = "TrackingFiles/".$originalFileName."-AtHome_TrackingFile-". $month. "-" . $day. "-" . $year . ".csv";
        $handle = fopen($trackingFileName, 'wb');
        //create a .txt from updated original fileData
        ...
        fclose($handle);
        $_SESSION['trackingFile'] = $trackingFileName;
        $_SESSION['exceptionFile'] = $exFileName;
